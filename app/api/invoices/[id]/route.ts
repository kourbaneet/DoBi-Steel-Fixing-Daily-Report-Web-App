...
import { updateInvoiceSchema } from "@/modules/invoice/validations"
import { INVOICE_CONSTANTS } from "@/modules/invoice/constants"

// PATCH handler - Update invoice details (admin only)
const updateInvoiceHandler = async (req: ApiRequest, { params }: { params: { id: string } }) => {
  try {
    const result = await InvoiceService.updateInvoice(params.id, req.validatedBody, req.user)
