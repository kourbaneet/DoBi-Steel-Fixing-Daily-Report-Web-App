...
// GET handler - Export payments history as CSV/XLSX
const exportPaymentsHistoryHandler = async (req: ApiRequest) => {
  try {
    ...
    const csvContent = await HistoryService.exportPaymentsHistory(params, req.user)

    const filename = `payments-history-${new Date().toISOString().split('T')[0]}.csv`
    ...
    console.error("Error exporting payments history:", error)
